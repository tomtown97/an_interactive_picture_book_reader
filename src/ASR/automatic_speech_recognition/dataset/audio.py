...
class Audio(Dataset):
    ...
    def from_csv(cls, file_path: str, **kwargs):
        ...
        comma separated. """
        references = pd.read_csv(file_path, usecols=['filename', 'text'],
                                 sep=',', encoding='utf-8', header=0)
        references.columns = ['path', 'transcript']
    ...
    def get_batch(self, index: int) -> Tuple[List[np.ndarray], List[str]]:
        ...
        paths, transcripts = [os.path.join('data', path) for path in references.path], references.transcript.tolist()
