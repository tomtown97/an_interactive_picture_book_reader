...
text_paths = pd.read_table('train.tsv')
pathss = text_paths['path'].values

textss = text_paths['sentence'].values
texts = []
paths = []
for lines in textss:
    line = lines.replace('，', '').replace('。', '').replace('“', '').replace('”', '').replace('《', '').replace('》',
                                                                                                              '').replace(
...
np.save("haha.npy", features)
# features = np.load("haha.npy")
...
model.compile(loss={'ctc': lambda ctc_true, ctc_pred: ctc_pred}, optimizer="adam")

checkpointer = ModelCheckpoint(filepath='asr.h5', verbose=0)
lr_decay = ReduceLROnPlateau(monitor='loss', factor=0.002, patience=1, min_lr=0.000)
...
sub_model = load_model('asr.h5', compile=False)
